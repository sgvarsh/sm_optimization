...
class Apriori:
    ...
    def generate_more(self, freq_sets, k, memo, minsupport):
        ...
        print ("reduced_candidate_list", len(reduced_candidate_list), " full list ",
               len(initial_list))
    ...
    def record_output(self, final_map, output_file):
        ...
        with open(output_file, "w") as fw:
            ...
            for tup in reversed(sorted(final_map.items(), key=lambda x: x[1])):
                ...
                fw.write((str(len(tup[0]))) + "," + str(tup[1]) + "," + str(list_form) + "\n")
...
if __name__ == '__main__':
    ...
    if options.input is not None:
        ...
        input = "sample.dat" # test file = "testcase.txt"
